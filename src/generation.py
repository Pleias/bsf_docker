...
if __name__ == "__main__":
        # Simple manual test
    rag = GenerationEngine(model_path_or_name=r"C:\Users\User\Documents\Pleias\bsf_docker\models\1b_rag.gguf",
                        max_tokens=2048,
                        backend="llama_cpp")
    ...
    sources = [
        ...
        },
        {
            ...
            "metadata": {"source": "Historical Monuments Guide", "year": 1995}
        },
        {
            "text": "France is a country located in Western Europe with several overseas territories and regions. The metropolitan area of France extends from the Mediterranean Sea to the English Channel and the North Sea, and from the Rhine to the Atlantic Ocean. France is one of the world's oldest nations and a leader in cultural development, having produced many influential artists, writers, and philosophers. The country has a long history of significant political and military accomplishments, and remains a cultural and political power in the modern world.",
            "metadata": {"source": "Wikipedia", "last_updated": "2023-04-15"}
        },
        {
            "text": "The Louvre is a famous museum in Paris, housed in the Louvre Palace, which was originally built as a fortress in the late 12th century under Philip II. The museum opened on August 10, 1793, during the French Revolution, and has since become the world's largest art museum and a historic monument in Paris. The Louvre's collection spans work from ancient civilizations to the mid-19th century. It exhibits over 35,000 objects from prehistory to the 21st century over an area of 60,600 square meters (652,000 sq ft). In 2019, the Louvre received 9.6 million visitors, making it the most visited museum in the world.",
            "metadata": {"source": "Art Museums of the World", "year": 2020, "author": "Dr. Sophia Laurent"}
        }
    ]
    ...
    print("\nProcessed Response:")
    for section_name, section_content in response["processed"].items():
        print(f"\n--- {section_name.upper()} ---")
        print(section_content)

    # Specifically check for and display clean_answer and citations
    if 'clean_answer' in response["processed"]:
        print("\n--- CLEAN_ANSWER ---")
        print(response["processed"]["clean_answer"])

    if 'citations' in response["processed"]:
        print("\n--- CITATIONS ---")
        for citation in response["processed"]["citations"]:
            print(f"Citation {citation['citation_number']}: Source {citation['source_id']}")
            print(f"Cited text: {citation['cited_text']}")
            print(f"Supported text: {citation['supported_text']}")
            print()

    print("\nFormatted Prompt:")
    print(rag.format_prompt(query, sources))

    # Save the response as JSON
    json_output = rag.to_json(response)